...
class Model_WideDeep(Model):
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        with tf.name_scope('Metric'):
            self.target_ph = self.y_ph
            self.loss = -tf.reduce_mean(tf.log(tf.clip_by_value(self.y_hat, 1e-9, 1.0)) * self.target_ph)
            self.optimizer = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss)
...
class Model_DeepFM(Model):
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        state_action_feature = tf.reduce_sum(state_action_feature, 1)
        state_dnn1 = tf.layers.dense(state_action_feature, EMBEDDING_DIM * 2, activation=None, name='state_feature_f1')
        state_dnn1 = prelu(state_dnn1, 'state_feature_p1')
        state_dnn2 = tf.layers.dense(state_dnn1, EMBEDDING_DIM, activation=None, name='state_feature_f2')
        ...
        d_layer_f1 = tf.concat([self.ad_eb, self.state_eb], -1)
        d_layer_f1 = tf.layers.dense(d_layer_f1, 1, activation=None, name='wide')
        d_layer_f2 = tf.reduce_sum(self.ad_eb * self.state_eb, -1)
        d_layer_f2 = tf.expand_dims(d_layer_f2, -1)
        d_layer_fm = d_layer_f1 + d_layer_f2
        self.y_hat = tf.nn.sigmoid(dnn3 + d_layer_fm)
...
class Model_DIN(Model):
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        with tf.name_scope('State_action_feature_embedding'):
            ...
            self.ad_his_eb = tf.tile(self.ad_eb, [1, tf.shape(self.effect_batch_his_embedded)[1]])
            self.ad_his_eb = tf.reshape(self.ad_his_eb, [tf.shape(self.ad_eb)[0], -1, EMBEDDING_DIM])
            ...
            state_action_dnn1 = tf.layers.dense(state_action_feature, EMBEDDING_DIM * 2, activation=None, name='state_action_f1')
            state_action_dnn1 = prelu(state_action_dnn1, 'state_action_p1')
            state_action_dnn2 = tf.layers.dense(state_action_dnn1, EMBEDDING_DIM, activation=None, name='state_action_f2')
...
class Model_RNN(Model):
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        inp = tf.concat([self.ad_eb, state_action_feature], -1)
        rnn_outputs, _ = tf.nn.dynamic_rnn(GRUCell(EMBEDDING_DIM), inputs=inp, dtype=tf.float32, scope='gru')
        rnn_summary = tf.reduce_sum(rnn_outputs, -2)
        self.y_hat = tf.nn.sigmoid(tf.reduce_sum(rnn_summary, -1))
        self.y_hat = tf.reshape(self.y_hat, [-1, 1])
        self.y_hat = tf.concat([self.y_hat, 1.0 - self.y_hat], -1)
...
class Model_biRNN(Model):
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        self.ad_eb = tf.reshape(self.ad_eb, [tf.shape(self.ad_eb)[0], -1, EMBEDDING_DIM])
        inp = tf.concat([self.ad_eb, state_action_feature], -1)
        rnn_outputs, _ = tf.nn.bidirectional_dynamic_rnn(GRUCell(EMBEDDING_DIM), GRUCell(EMBEDDING_DIM), inputs=inp, dtype=tf.float32, scope='gru')

        rnn_summary = tf.reduce_sum(rnn_outputs[0], -2) + tf.reduce_sum(rnn_outputs[1], -2)
        rnn_summary = tf.reduce_sum(rnn_summary, -1)
        self.y_hat = tf.nn.sigmoid(rnn_summary)
        self.y_hat = tf.reshape(self.y_hat, [-1, 1])
        self.y_hat = tf.concat([self.y_hat, 1.0 - self.y_hat], -1)
...
class Model_DSPN(Model):
    ...
    def DAVN_att_fcn_attention(self, query, facts, mask, EMBEDDING_DIM, type='null'):
        ...
        att_rnn, _ = tf.nn.bidirectional_dynamic_rnn(GRUCell(EMBEDDING_DIM), GRUCell(EMBEDDING_DIM), inputs=query, \
                                       dtype=tf.float32, scope='gru_att' + type)
        query = att_rnn[0] + att_rnn[1]
        queries = tf.tile(query, [1, 1, tf.shape(facts)[2]])
        queries = tf.reshape(queries, tf.shape(facts))

        feature_all = tf.concat([queries, facts, queries-facts, queries*facts], axis=-1)
        d_layer_1_all = tf.layers.dense(feature_all, 80, activation=tf.nn.sigmoid, name='f1_att' + type)
        d_layer_2_all = tf.layers.dense(d_layer_1_all, 40, activation=tf.nn.sigmoid, name='f2_att' + type)
        d_layer_3_all = tf.layers.dense(d_layer_2_all, 1, activation=None, name='f3_att' + type)
        d_layer_3_all = tf.reshape(d_layer_3_all, [-1, tf.shape(facts)[1], 1, tf.shape(facts)[2]])
        scores = d_layer_3_all

        key_masks = tf.equal(mask, tf.ones_like(mask))
    ...
    def DAVN_att_ad_attention(self, query, facts, mask, EMBEDDING_DIM, type='null'):
        ...
        # query = ad_att_dnn2 # batch * day * embedding
    ...
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        with tf.name_scope('direct_actions'):
            self.direct_action_info = self.DAVN_att_ad_attention(self.ad_eb, self.direct_action_batch_his_embedded, self.direct_action_batch_his_mask_ph, EMBEDDING_DIM, type='direct')

        with tf.name_scope('pos_actions'):
            self.pos_action_info = self.DAVN_att_ad_attention(self.ad_eb, self.pos_action_batch_his_embedded, self.pos_action_batch_his_mask_ph, EMBEDDING_DIM, type='pos')
        ...
        with tf.name_scope('rnn'):
            ...
            self.ad_his_eb = tf.tile(self.ad_eb, [1, tf.shape(self.effect_batch_his_embedded)[1]])
            self.ad_his_eb = tf.reshape(self.ad_his_eb, [tf.shape(self.ad_eb)[0], -1, EMBEDDING_DIM])
            self.state_action_info = tf.concat([self.ad_his_eb, self.effect_batch_his_embedded, self.pos_batch_his_embedded, self.davn_direct_batch_his_embedded, self.action_info], -1)
            
            bi_rnn_outputs1, bi_rnn_states1 = tf.nn.bidirectional_dynamic_rnn(GRUCell(EMBEDDING_DIM), GRUCell(EMBEDDING_DIM), inputs=self.state_action_info, \
...
class Model_DSPN_RNN(Model):
    ...
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        with tf.name_scope('rnn'):
            ...
            rnn_outputs1, rnn_states1 = tf.nn.dynamic_rnn(GRUCell(EMBEDDING_DIM), inputs=self.state_action_info, \
                                       dtype=tf.float32, scope='gru1')
...
class Model_DSPN_MLP(Model):
    ...
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        with tf.name_scope('rnn'):
            ...
            dnn1 = tf.layers.dense(self.state_action_info, 200, activation=None, name='f1')
            dnn1 = prelu(dnn1, 'prelu1')
            dnn2 = tf.layers.dense(dnn1, 80, activation=None, name='f2')
            dnn2 = prelu(dnn2, 'prelu2')
...
class Model_DSPN_no_att(Model):
    ...
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        with tf.name_scope('rnn'):
            ...
            self.ad_his_eb = tf.tile(self.ad_eb, [1, tf.shape(self.effect_batch_his_embedded)[1]])
            self.ad_his_eb = tf.reshape(self.ad_his_eb, [tf.shape(self.ad_eb)[0], -1, EMBEDDING_DIM])
            self.state_action_info = tf.concat([self.ad_his_eb, self.effect_batch_his_embedded, self.pos_batch_his_embedded, self.davn_direct_batch_his_embedded, self.mlp_direct_action_batch_his_embedded, self.mlp_pos_action_batch_his_embedded], -1)
...
class Model_DSPN_noID(Model):
    ...
    def DAVN_att_ad_attention(self, facts, mask, EMBEDDING_DIM, type='null'):
        # facts: batch * day * maxlen * embedding
        # query: batch * embedding
        ...
        tmp_direct_batch_his_mask_ph = tf.expand_dims(self.direct_batch_his_mask_ph, -1)
        self.davn_direct_batch_his_embedded = tf.reduce_sum(tmp_direct_batch_his_mask_ph * self.direct_batch_his_embedded, 2)
        query = tf.concat([self.effect_batch_his_embedded, self.davn_direct_batch_his_embedded, self.pos_batch_his_embedded], -1)
        att_rnn, _ = tf.nn.bidirectional_dynamic_rnn(GRUCell(EMBEDDING_DIM), GRUCell(EMBEDDING_DIM), inputs=query, \
                                       dtype=tf.float32, scope='gru_att' + type)
        query = att_rnn[0] + att_rnn[1]
        # with tf.name_scope('DAVN_att_ad_attention_ad_feature_embedding'):
    ...
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        with tf.name_scope('direct_actions'):
            self.direct_action_info = self.DAVN_att_ad_attention(self.direct_action_batch_his_embedded, self.direct_action_batch_his_mask_ph, EMBEDDING_DIM, type='direct')

        with tf.name_scope('pos_actions'):
            self.pos_action_info = self.DAVN_att_ad_attention(self.pos_action_batch_his_embedded, self.pos_action_batch_his_mask_ph, EMBEDDING_DIM, type='pos')
...
class Model_DSPN_noReport(Model):
    ...
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        with tf.name_scope('rnn'):
            ...
            self.ad_his_eb = tf.tile(self.ad_eb, [1, tf.shape(self.effect_batch_his_embedded)[1]])
            self.ad_his_eb = tf.reshape(self.ad_his_eb, [tf.shape(self.ad_eb)[0], -1, EMBEDDING_DIM])
            self.state_action_info = tf.concat([self.ad_his_eb, self.pos_batch_his_embedded, self.davn_direct_batch_his_embedded, self.action_info], -1)
...
class Model_DSPN_attention(Model):
    ...
    def __init__(self, n_adgroup, n_member, n_campaign, \
        ...
        with tf.name_scope('Ad_feature_embedding'):
            ...
            self.state_action_info = tf.concat([self.action_info], -1)
...
class Model_DSPN_no_attention(Model):
    def __init__(self, n_adgroup, n_member, n_campaign, \
                       n_item, n_cate, n_commodity, \
                       n_node, n_effect, n_pos, n_direct, \
                       n_direct_action, n_pos_action, \
        ...
        with tf.name_scope('rnn'):
            ...
            self.state_action_info = tf.concat([self.mlp_direct_action_batch_his_embedded, self.mlp_pos_action_batch_his_embedded], -1)
